...
if(isset($_SESSION["Cart"])) //if a cart variable exists
{
    ...
    if ($result->num_rows > 0) {
        ...
        /*echo "
        <div class='row'>
        <div style='height: 60vh; width: 100%; display: flex; justify-content: center; align-items: center; flex-direction: column; padding: 5% 5% 1% 1%; height: fit-content;'>
          <div class='progress-checkout-container' style='display: flex; justify-content: space-between; align-items: center; width: 60%'>
              <div class='progress-step-container' style='display: flex; justify-content: center; align-items: center; flex-direction: column;'>
                  <div class='step-check' style='background-color: rgb(0,132,255); height: 30px; width: 30px; border-radius: 30px; margin-bottom: 1rem; position: relative; background-image: url(Resources/check.svg); backgroud-position: center; background-repeat: no-repeat; background-size: 60%;'></div>
                  <span class='step-title' style='color: grey;'>Delivery</span>
              </div>
              <div class='progress-step-container' style='display: flex; justify-content: center; align-items: center; flex-direction: column;'>
                  <div class='step-check' style='background-color: white; border: 2px solid grey; height: 30px; width: 30px; border-radius: 30px; margin-bottom: 1rem; position: relative; background-image: url(Resources/check.svg); backgroud-position: center; background-repeat: no-repeat; background-size: 60%;'></div>
                  <span class='step-title' style='color: grey;'>Order Preview</span>
              </div>
              <div class='progress-step-container' style='display: flex; justify-content: center; align-items: center; flex-direction: column;'>
                  <div class='step-check' style='background-color: white; border: 2px solid grey; height: 30px; width: 30px; border-radius: 30px; margin-bottom: 1rem; position: relative; background-image: url(Resources/check.svg); backgroud-position: center; background-repeat: no-repeat; background-size: 60%;'></div>
                  <span class='step-title' style='color: grey;'>Delivery & Payment</span>
              </div>
          </div>
         </div>
         </div>
    ...
    echo "
        <div class='col-sm-6'>
            ...
            <div class='checkout-form' style='display: flex; justify-content: center; align-items: flex-start; flex-direction: column; width: 100%;'>
                ...
                <div class='input-line' style='display: flex; justify-content: center; align-items: flex-start; margin-bottom: 2rem; width: 100%;'>
                    <label for='name' style='padding-right: 10%; font-size: 16px; color: grey; margin-bottom: 0.5rem;'>Delivery Mode&nbsp;<abbr style='color: red;'>*</abbr>&emsp;&emsp;&emsp;&emsp;&emsp;</label>
                    <input type='radio' onclick='check()' style='width: 10%; height: 20px; padding: 0 10px; background-color: #f2f2f2; border: none;' id='DeliveryMode' name='delivery' value='2'>
...
<script>
    function check() {
        ...
        document.getElementById("totalCost").innerHTML = '$' + (Number.parseFloat(document.getElementById("subTotal").innerText.substr(1),10) + Number.parseFloat(document.querySelector('input[name="delivery"]:checked').value, 10)).toString();
        document.getElementById("FinalTotal").innerHTML = '$' + (Number.parseFloat(document.getElementById("subTotal").innerText.substr(1),10) + Number.parseFloat(document.querySelector('input[name="delivery"]:checked').value, 10)).toString();
    };
</script>
<?php
